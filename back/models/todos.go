...
type Todo struct {
	...
	CreatedUserID int       `boil:"created_user_id" json:"created_user_id" toml:"created_user_id" yaml:"created_user_id"`
...
var TodoTableColumns = struct {
	...
	ID:            "todos.id",
	Title:         "todos.title",
	CreatedUserID: "todos.created_user_id",
...
func (r *todoR) GetCreatedUser() *User {
	if r == nil {
		return nil
	}
...
var (
	todoAllColumns            = []string{"id", "title", "created_user_id", "created_at", "updated_at"}
	todoColumnsWithoutDefault = []string{"title", "created_user_id"}
...
func (o *Todo) CreatedUser(mods ...qm.QueryMod) userQuery {
...
func (o *Todo) DoneTodos(mods ...qm.QueryMod) doneTodoQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"done_todos\".\"todo_id\"=?", o.ID),
	)
...
func (todoL) LoadCreatedUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTodo interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.CreatedUserTodos = append(foreign.R.CreatedUserTodos, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.CreatedUserID == foreign.ID {
				...
				foreign.R.CreatedUserTodos = append(foreign.R.CreatedUserTodos, local)
				break
			}
		}
	}
...
func (todoL) LoadDoneTodos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeTodo interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`done_todos`),
		qm.WhereIn(`done_todos.todo_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Todo) SetCreatedUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	var err error
	if insert {
	...
	if !cached {
		...
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(todoPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert todos, could not build conflict column list")
			}
