...
func testTodoToManyDoneTodos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := TodoSlice{&a}
	if err = a.L.LoadDoneTodos(ctx, tx, false, (*[]*Todo)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTodoToManyAddOpDoneTodos(t *testing.T) {
	var err error
...
func testTodoToOneUserUsingCreatedUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	local.CreatedUserID = foreign.ID
	...
	if err = local.L.LoadCreatedUser(ctx, tx, false, (*[]*Todo)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTodoToOneSetOpUserUsingCreatedUser(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		...
		if x.R.CreatedUserTodos[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if a.CreatedUserID != x.ID {
		...
		if a.CreatedUserID != x.ID {
			t.Error("foreign key was wrong value", a.CreatedUserID, x.ID)
		}
	}
}
...
var (
	todoDBTypes = map[string]string{`ID`: `integer`, `Title`: `character varying`, `CreatedUserID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
