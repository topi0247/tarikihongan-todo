...
func testUserToManyDoneTodos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, doneTodoDBTypes, false, doneTodoColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, doneTodoDBTypes, false, doneTodoColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.DoneTodos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadDoneTodos(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DoneTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DoneTodos = nil
	if err = a.L.LoadDoneTodos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DoneTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyCreatedUserTodos(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	b.CreatedUserID = a.ID
	c.CreatedUserID = a.ID
	...
	check, err := a.CreatedUserTodos().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	bFound, cFound := false, false
	for _, v := range check {
		if v.CreatedUserID == b.CreatedUserID {
			bFound = true
		}
		if v.CreatedUserID == c.CreatedUserID {
			cFound = true
		}
	}
	...
	slice := UserSlice{&a}
	if err = a.L.LoadCreatedUserTodos(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedUserTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.CreatedUserTodos = nil
	if err = a.L.LoadCreatedUserTodos(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.CreatedUserTodos); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testUserToManyAddOpDoneTodos(t *testing.T) {
	var err error
	...
	var a User
	var b, c, d, e DoneTodo

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, userDBTypes, false, strmangle.SetComplement(userPrimaryKeyColumns, userColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*DoneTodo{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, doneTodoDBTypes, false, strmangle.SetComplement(doneTodoPrimaryKeyColumns, doneTodoColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*DoneTodo{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDoneTodos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.DoneTodos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DoneTodos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DoneTodos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testUserToManyAddOpCreatedUserTodos(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddCreatedUserTodos(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.ID != first.CreatedUserID {
			t.Error("foreign key was wrong value", a.ID, first.CreatedUserID)
		}
		if a.ID != second.CreatedUserID {
			t.Error("foreign key was wrong value", a.ID, second.CreatedUserID)
		}

		if first.R.CreatedUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}
		if second.R.CreatedUser != &a {
			t.Error("relationship was not added properly to the foreign slice")
		}

		if a.R.CreatedUserTodos[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.CreatedUserTodos[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.CreatedUserTodos().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
var (
	userDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `UID`: `character varying`, `Provider`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
